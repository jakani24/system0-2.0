<?php
	...
	if(isset($_GET["token"])){
		if($_GET["token"]==$_SESSION["creation_token"]){
			...
			header("LOCATION: /login/login.php?acc_verify_ok");
		}else{
			$login_err = "Dein Link ist entweder abgelaufen oder ungültig. Erzeuge einen neuen, in dem du auf <a href='/login/login.php?resend_acc_verify'>diesen Link</a> klickst.";
